...
function AddAccountStatusModal(props) {
  const { formatMessage: covert } = useIntl();
  const dispatch = useDispatch();
  // console.log(props.specialLists);
  const handleSubmit = (values) => {
    if (props?.data?._id) {
      dispatch(UpdateAccountStatusEnum(values, props?.data?._id));
    } else {
      dispatch(addAccountStatusEnum(values));
    }
    ...
    >
      <Modal.Body className="p-0 ">
        <div className="add-new-bene institution">
          <h4>{covert({ id: "Add New Account Status" })}</h4>
          <Row>
            <Col md={12}>
              <Formik
                initialValues={{
...
export default AddAccountStatusModal;
