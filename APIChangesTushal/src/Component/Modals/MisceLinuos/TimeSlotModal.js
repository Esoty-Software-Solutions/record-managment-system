...
import { UpdateTimeSlotEnum, addTimeSlotEnum } from "../../../redux/actions/action";
...
function TimeSlotModal(props) {
  ...
  const handleSubmit = (values) => {
    if (props?.data?._id) {
      dispatch(UpdateTimeSlotEnum(values, props?.data?._id));
    } else {
      dispatch(addTimeSlotEnum(values));
    }
    ...
    >
      <Modal.Body className="p-0 ">
        <div className="add-new-bene institution">
          <h4> {covert({ id: "Add New TimeSlot Enum" })}</h4>
          <Row>
            <Col md={12}>
              <Formik
                initialValues={{
